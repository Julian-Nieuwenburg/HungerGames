...
public class Invantory : MonoBehaviour
{
    ...
    private Canvas invantoryCanvas;
    ...
    void Start()
    {
        ...
        // Find the Canvas and set its render mode and camera
        invantoryCanvas = invantoryRoot.GetComponent<Canvas>();
        if (invantoryCanvas != null)
        {
            invantoryCanvas.renderMode = RenderMode.ScreenSpaceCamera;
            invantoryCanvas.worldCamera = playerCamera;
        }
        else
        {
            Debug.LogError("Invantory Canvas not found!");
        }
    }

    // Update the input logic here to change how the slots get toggled and the selected item is used.
    void Update()
    {
